#!/usr/bin/env python3
"""One-shot, source-reviewable wrapper for the future Edge actor.

This module deliberately owns no runtime, certificate, or network setup.  It
only verifies a separately supplied binding and authorization, claims an
exclusive guard, then performs one non-shell child invocation and records a
final result without retry or replay.
"""
...
def _reject_existing_guard(path: Path) -> None:
    try:
        path.lstat()
    except FileNotFoundError:
        return
    except OSError as exc:
        raise ContractError(f"cannot stat guard: {exc}") from exc
    _check_private_regular(path, "guard", expected_mode=0o600)
    raise AlreadyClaimed("exclusive producer guard already exists")


def _write_atomic_result(path: Path, value: Mapping[str, Any]) -> None:
...
def run_once(
    *,
    ...
    binding_reference, binding_hash = _require_path_reference(contract, "binding", binding_path)
    del binding_reference
    ...
    _reject_existing_guard(guard_path)
    ...
    _write_atomic_result(result_path, result_record)
