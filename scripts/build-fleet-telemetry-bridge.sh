...
PATCH_SHA256=828a98fe3b36bf07c3d2d32041be45efad7594c6c580a982b588875a10898b98
GO_VERSION=go1.27.0
...
if [ -n "${GO_BINARY:-}" ]; then
    case "$GO_BINARY" in /*) ;; *) die "GO_BINARY must be absolute" ;; esac
    [ -f "$GO_BINARY" ] && [ -x "$GO_BINARY" ] && [ ! -L "$GO_BINARY" ] \
        || die "GO_BINARY is not a regular executable"
    go_parent=$(CDPATH='' cd "$(dirname "$GO_BINARY")" && pwd -P)
    GO="$go_parent/$(basename "$GO_BINARY")"
    [ "$GO" = "$GO_BINARY" ] || die "GO_BINARY must be canonical"
    allowed=0
    case "$GO" in /opt/homebrew/*) allowed=1 ;; esac
    if [ -n "${RUNNER_TOOL_CACHE:-}" ]; then
        case "$RUNNER_TOOL_CACHE" in /*) ;; *) die "RUNNER_TOOL_CACHE must be absolute" ;; esac
        [ -d "$RUNNER_TOOL_CACHE" ] && [ ! -L "$RUNNER_TOOL_CACHE" ] \
            || die "RUNNER_TOOL_CACHE is missing or unsafe"
        tool_root=$(CDPATH='' cd "$RUNNER_TOOL_CACHE" && pwd -P)
        case "$GO" in "$tool_root"/*) allowed=1 ;; esac
    fi
    [ "$allowed" -eq 1 ] || die "GO_BINARY is outside approved tool roots"
else
